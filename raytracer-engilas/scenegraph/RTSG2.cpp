...
#define MAX_MESH_TRIANGLES 256 // Fits in 64KB UBO limit
...
struct rt_triangle {
    ...
    float b0[4]; float b1[4]; float b2[4];
};
    ...
    bool parse(std::istream& file) {
        ...
        while (std::getline(file, line)) {
            ...
            if (prefix == "v") { Vec3 v; ss >> v.x >> v.y >> v.z; positions.push_back(v); }
                ...
                while (ss >> v[count] && count < 4) count++;
...
struct rt_triangle { vec4 v0, v1, v2; vec4 n0, n1, n2; vec4 uv0, uv1, uv2; vec4 t0, t1, t2; vec4 b0, b1, b2; };
...
bool intersectTriangle(vec3 ro, vec3 rd, rt_triangle tri, float tmin, out float t, out vec3 bar, out vec3 normal) {
...
float calcInter(vec3 ro, vec3 rd, out int num, out int type, out rt_triangle outTri, out vec3 outBar) {
    ...
    for(int i = 0; i < scene.mesh_count; i++) {
        if(intersectTriangle(ro, rd, mesh_tris[i], tmin, t, bar, normal)) {
            num = i; tmin = t; type = TYPE_MESH; outTri = mesh_tris[i]; outBar = bar;
        }
    }
...
float inShadow(vec3 ro, vec3 rd, float dist) {
    ...
    if(shadow == 0) for(int i = 0; i < scene.mesh_count; i++) if(intersectTriangle(ro, rd, mesh_tris[i], dist, t, bar, normal)) { shadow = 1; break; }
    return shadow;
}
...
hit_record get_hit_info(vec3 ro, vec3 rd, vec3 pt, float t, int num, int type, rt_triangle tri, vec3 bar) {
    ...
    if(type == TYPE_MESH) {
        ...
        vec3 B = tri.b0.xyz * bar.x + tri.b1.xyz * bar.y + tri.b2.xyz * bar.z;
        
        n = normalize(n); T = normalize(T); B = normalize(B);
        mat3 TBN = mat3(T, B, n);
        
        // Procedural Bump Map (Checkerboard) since we don't have an external image loader
        float checker = mod(floor(uv.x * 10.0) + floor(uv.y * 10.0), 2.0);
        vec3 bumpN = vec3(0.0, 0.0, 1.0);
        if (checker > 0.5) bumpN = normalize(vec3(0.4, 0.0, 1.0));
...
void main() {
    ...
    int type = 0, num; hit_record hr; rt_triangle tri_hit; vec3 bar_hit;
