// Raytracer (engilas) — Scenegraph, Godrays, Fog, Water, Multiple Lights
...
// RENAMED from rotate to quatRotate to avoid shadowing the member function
...
public:
    ...
    // FIXED: Added default empty implementation {} so SceneNode is no longer abstract
    ...
    void traverse(std::vector<rt_sphere>& sph, std::vector<rt_box>& bx, std::vector<rt_plane>& pl, 
                  ...
                  std::vector<rt_light_area>& la, std::vector<rt_light_direct>& ld,
                  Vec3 pPos, Quat pRot, Vec3 pScl, Vec3 pTint) {
        ...
        flatten(sph, bx, pl, wt, lp, ls, la, ld, gPos, gRot, gScl, gTint);
        for(auto c : children) {
            c->traverse(sph, bx, pl, wt, lp, ls, la, ld, gPos, gRot, gScl, gTint);
        }
    }
};
...
class PlaneNode : public SceneNode {
    Vec3 normal; rt_material mat;
public:
    PlaneNode(Vec3 n, rt_material m) : normal(n), mat(m) {}
    void flatten(std::vector<rt_sphere>&, std::vector<rt_box>&, std::vector<rt_plane>& pl, 
                 std::vector<rt_water>&, std::vector<rt_light_point>&, std::vector<rt_light_spot>&,
                 std::vector<rt_light_area>&, std::vector<rt_light_direct>&,
                 Vec3 gPos, Quat gRot, Vec3 gScl, Vec3 gTint) override {
        rt_plane p = {};
        p.pos[0] = gPos.x; p.pos[1] = gPos.y; p.pos[2] = gPos.z;
        Vec3 n = normalize(quatRotate(gRot, normal));
        p.normal[0] = n.x; p.normal[1] = n.y; p.normal[2] = n.z;
        p.material = mat;
        p.material.color[0] *= gTint.x; p.material.color[1] *= gTint.y; p.material.color[2] *= gTint.z;
        if(pl.size() < MAX_PLANES) pl.push_back(p);
    }
};
...
public:
    ...
    void flatten(std::vector<rt_sphere>&, std::vector<rt_box>&, std::vector<rt_plane>&, 
                 std::vector<rt_water>&, std::vector<rt_light_point>&, std::vector<rt_light_spot>&,
                 std::vector<rt_light_area>& la, std::vector<rt_light_direct>&,
                 Vec3 gPos, Quat gRot, Vec3 gScl, Vec3 gTint) override {
        rt_light_area l = {};
        l.pos[0] = gPos.x; l.pos[1] = gPos.y; l.pos[2] = gPos.z;
...
public:
    ...
    void flatten(std::vector<rt_sphere>&, std::vector<rt_box>&, std::vector<rt_plane>&, 
                 std::vector<rt_water>&, std::vector<rt_light_point>&, std::vector<rt_light_spot>&,
                 std::vector<rt_light_area>&, std::vector<rt_light_direct>& ld,
                 Vec3 gPos, Quat gRot, Vec3 gScl, Vec3 gTint) override {
        rt_light_direct l = {};
...
// ---------------------------------------------------------
// Helper functions to build materials
// ---------------------------------------------------------
...
struct rt_scene { vec4 quat_camera_rotation; vec3 camera_pos; vec3 bg_color; int canvas_width; int canvas_height; int reflect_depth; int sphere_count; int box_count; int plane_count; int water_count; int light_point_count; int light_spot_count; int light_area_count; int light_direct_count; float time; };
...
hit_record get_hit_info(vec3 ro, vec3 rd, vec3 pt, float t, int num, int type) {
    ...
    if(type == TYPE_SPHERE) {
        hr = hit_record(
            spheres[num].mat,
            normalize(pt - spheres[num].obj.xyz),
            0.0,
            1.0
        );
    }
    ...
    if(type == TYPE_WATER) {
        ...
        float wave =
            sin(p.x * waters[num].waveFreq + scene.time * waters[num].flowSpeed) *
    ...
    hr.bias_mult = (9e-3 * length(pt - ro) + 35.0) / 35000.0;
...
vec3 applyFog(vec3 ro, vec3 rd, float dist, vec3 hitColor) {
    ...
    for(int i = 0; i < 8; i++) {
        ...
        if(fog.enableGodrays == 1 && fog.godrayLightIndex < scene.light_direct_count) {
            ...
            inscatter += (1.0 - shadow) * lights_direct[fog.godrayLightIndex].color * density * stepSize * transmittance;
        } else {
            inscatter += fog.color * density * stepSize * transmittance;
        }
    }
    ...
    for(int i = 0; i < ITERATIONS; i++) {
        ...
        if(tm < maxDist) {
            ...
            if(mat.refraction > 1.0) {
                ...
                ro = pt + n * hr.bias_mult;
                color += localColor * refractMultiplier * mask;
                rd = reflect(rd, n); mask *= reflectMultiplier;
            } else {
                color += localColor * mask * hr.alpha;
                break;
            }
        } else {
...
void init_buffers() {
    ...
    init_buffer(&fogUbo, "fog_buf", 9, sizeof(rt_fog), NULL);
}
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        root->traverse(spheres, boxes, planes, waters, lights_point, lights_spot, lights_area, lights_direct, pPos, pRot, pScl, pTint);
