...
GLuint sceneUbo = 0;
GLuint sphereUbo = 0; // <-- ADD THIS LINE
GLuint boxUbo = 0;    // <-- ADD THIS LINE
...
void init_buffers() {
    // Fixed the lambda signature which was truncated in the original code
    auto init_buffer = [&](GLuint* ubo, const char* name, GLuint bindingPoint, size_t size, const void* data) {
        glGenBuffers(1, ubo);
    ...
    GLuint planeUbo, surfaceUbo, torusUbo, ringUbo, lightPointUbo, lightDirectUbo;
    ...
    init_buffer(&ringUbo, "rings_buf", 6, sizeof(rt_ring), NULL);
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Animate sphere 0 (orbiting in the XZ plane)
        spheres[0].obj[0] = 2.0f + 2.0f * sinf(t);
        spheres[0].obj[2] = 6.0f + 2.0f * cosf(t);

        // Animate sphere 1 (pulsing radius)
        spheres[1].obj[3] = 1.0f + 0.3f * sinf(t * 3.0f);

        // Animate sphere 2 (bouncing up and down)
        spheres[2].obj[1] = 2.0f + 1.5f * sinf(t * 1.5f);

        // Animate box 1 (rotating around its Y-axis at 90 degrees per second)
        Quat q_box = angleAxis(radians(90.0f) * t, Vec3(0, 1, 0));
        boxes[1].quat_rotation[0] = q_box.x;
        boxes[1].quat_rotation[1] = q_box.y;
        boxes[1].quat_rotation[2] = q_box.z;
        boxes[1].quat_rotation[3] = q_box.w;
