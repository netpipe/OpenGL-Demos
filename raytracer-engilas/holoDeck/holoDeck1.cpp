...
public:
    ...
    void subdivide() {
        ...
        // Using explicit unique_ptr constructor for older C++ standard compatibility
...
// UBO Structs strictly aligned to 16-byte (vec4) boundaries for std140
struct Light {
    vec4 type_info; // .x contains type
    vec4 pos;
    vec4 dir;
    vec4 color;
    vec4 params; // intensity, range, cutoff, outerCutoff
    vec4 u;
    vec4 v;
};
...
vec3 calcLighting(vec3 pos, vec3 normal, vec3 albedo, vec3 viewDir) {
    ...
    for(int i=0; i<numLights; ++i) {
        ...
        else if (lType == 3) { // Area (Sampled Grid)
            vec3 sum = vec3(0.0);
            int samples = 4;
...
void main() {
    ...
    if(minT < 1e10) {
        ...
        for(int i=0; i<numLights; ++i) {
            if(int(lights[i].type_info.x) == 0) color += calcGodrays(hitPos, lights[i].dir.xyz, lights[i].color.xyz);
        }
...
// ============================================================================
// GLOBALS & ENGINE STATE (UBO std140 aligned structures)
// ============================================================================
...
RT_Sphere makeSphere(float x, float y, float z, float r, Vec3 col) {
    ...
    s.color[0] = col.x; s.color[1] = col.y; s.color[2] = col.z; s.color[3] = 1.0f;
    return s;
}
...
RT_Box makeBox(float x, float y, float z, Vec3 ext, Vec3 col) {
    ...
    b.color[0] = col.x; b.color[1] = col.y; b.color[2] = col.z; b.color[3] = 1.0f;
    return b;
}
...
RT_Plane makePlane(float x, float y, float z, Vec3 norm, Vec3 col) {
    ...
    p.color[0] = col.x; p.color[1] = col.y; p.color[2] = col.z; p.color[3] = 1.0f;
    return p;
}
...
int main() {
    ...
    // macOS strictly requires OpenGL 4.1 Core Profile and Forward Compat
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 4);
    glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 1);
    glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);
    glfwWindowHint(GLFW_OPENGL_FORWARD_COMPAT, GL_TRUE);
    
    GLFWwindow* window = glfwCreateWindow(wind_width, wind_height, "Holodeck Raytraced Scenegraph (GL 4.1)", NULL, NULL);
    ...
    // Create UBOs (Uniform Buffer Objects for OpenGL 4.1)
    ...
    // Bind UBOs to binding points (Required in GL 4.1 since layout binding=X is GL 4.2+)
    GLuint blockIndex;
