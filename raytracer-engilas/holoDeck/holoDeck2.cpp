...
public:
    ...
    float reflectivity = 0.1f; // <-- Added for raytraced reflections
    std::string objPath;
    GLuint VAO = 0;
    int indexCount = 0;
    AABB worldBounds;
...
void main() {
    ...
    // Recursive Reflection Loop (Max 4 bounces)
    for (int bounce = 0; bounce < 4; ++bounce) {
        ...
        if(hasWater == 1 && intersectWater(ro, rd, t, hitNormal)) {
            if(t < minT) { minT = t; hitPos = ro + rd*t; hitColor = vec3(0.1, 0.3, 0.8); hitWater = true; hitReflect = 0.6; }
        }
        ...
        if(minT < 1e10) {
            ...
            if (hitWater) {
                ...
                baseColor = mix(baseColor, skyColor, 0.4);
            }
            
            finalColor += baseColor * mask * (1.0 - hitReflect);
            
            if (hitReflect < 0.01) break;
            
            // Tint reflections by surface color
            mask *= mix(vec3(1.0), hitColor, 0.5) * hitReflect; 
    ...
    // Use first hit distance for depth buffer compositing
    if (firstHitT > 0.0) {
        vec3 firstHitPos = uCamPos + normalize(vViewDir) * firstHitT;
        mat4 viewProj = uProj * uView;
        vec4 clipPos = viewProj * vec4(firstHitPos, 1.0);
...
int main() {
    ...
    ground->reflectivity = 0.2f; 
    allNodes.push_back(ground);
    world.insert(ground);

    // Giant Mirror Sphere to prove reflections work
