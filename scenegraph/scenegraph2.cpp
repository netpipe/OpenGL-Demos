// ============================================================================
// Optimized Scenegraph & Culling Demo with Textured OBJ Loading
// ============================================================================
// Compile (Windows): g++ main.cpp -o demo -lglfw3 -lglew32 -lopengl32
// Compile (Linux):   g++ main.cpp -o demo -lglfw -lGLEW -lGL
//
// This single-file demo implements a hierarchical scenegraph with frustum 
// culling, combined with a fully functional OBJ/MTL loader. 
//
// FIXES APPLIED FOR TEXTURES:
// 1. Added a lightweight 24-bit BMP loader so no external libraries are needed.
// 2. Split rendering per-mesh so each material's specific texture is bound.
// 3. Added a procedural checkerboard fallback if texture files are missing/invalid.
// 4. Updated shaders to sample the texture and multiply by the base color.
// ============================================================================
...
// ==========================================
// 2. TEXTURE LOADER & PROCEDURAL FALLBACK
// ==========================================
GLuint loadTexture(const std::string& filename) {
    ...
    if (!file || filename.find("fallback") != std::string::npos || filename.empty()) {
        if (!filename.empty()) std::cout << "Texture file not found: " << filename << ". Generating checkerboard.\n";
    ...
    if (header[0] != 'B' || header[1] != 'M') {
        std::cout << "Not a valid BMP file: " << filename << ". Generating checkerboard.\n";
        file.close();
        unsigned char checker[64*64*4];
        for(int y=0; y<64; ++y) {
            for(int x=0; x<64; ++x) {
    ...
    for(int y=0; y<height; ++y) {
        for(int x=0; x<width; ++x) {
            int srcIdx = (height - 1 - y) * rowSize + x * 3;
            int dstIdx = (y * width + x) * 3;
            rgb[dstIdx + 0] = data[srcIdx + 2];
            rgb[dstIdx + 1] = data[srcIdx + 1];
            rgb[dstIdx + 2] = data[srcIdx + 0];
        }
    }
...
// ==========================================
// 4. SCENEGRAPH & RENDERABLE COMPONENTS
// ==========================================
...
struct Material {
    ...
    Material() : shininess(0), textureID(0), hasTexture(false) {}
};
...
struct GLMesh {
    ...
    void upload(const std::vector<Vertex>& verts, const OBJMaterial* mat) {
        if (mat) {
            ...
            material.textureID = mat->textureID;
            material.hasTexture = (mat->textureID != 0);
        }
    ...
    void draw() {
        if (vao) {
            ...
            glBindTexture(GL_TEXTURE_2D, material.hasTexture ? material.textureID : 0);
            
            glUniform1i(glGetUniformLocation(g_shaderProgram, "uTexture"), 0);
            glUniform1i(glGetUniformLocation(g_shaderProgram, "uHasTexture"), material.hasTexture ? 1 : 0);
...
void loadModelIntoRenderable(Renderable* r, const std::string& path) {
    ...
    if (model.load(path) && !model.meshes.empty()) {
        ...
        defaultMat.textureID = loadTexture("fallback_checkerboard");
        boxMesh->upload(verts, &defaultMat);
        r->addMesh(boxMesh);
    }
}
...
const char* fragmentShaderSource = R"(
    ...
    void main() {
        ...
        vec3 baseColor = uBaseColor * texColor;
...
int main() {
    ...
    GLMesh* boxMesh = new GLMesh();
    OBJMaterial boxMat;
    boxMat.textureID = loadTexture("fallback_checkerboard");
    boxMesh->upload(boxVerts, &boxMat);
    boxRenderable->addMesh(boxMesh);
    ...
    objNode->color = Vec3(1.0f, 1.0f, 1.0f); // Keep true color for textured objects
    ...
    for(int x=0; x<gridSize; ++x) {
        for(int z=0; z<gridSize; ++z) {
            ...
            boxNode->setRenderable(boxRenderable);
