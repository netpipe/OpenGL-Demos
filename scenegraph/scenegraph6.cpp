// ============================================================================
// Advanced Scenegraph Demo: Instancing, Materials, and Complex Lighting
// ============================================================================
...
// 5. Scene Node Color Tinting (multiplies with texture)
// 6. Comprehensive Light Nodes: Sun, Point, Spot, and 3 types of Area Lights!
// 7. FLASHLIGHT DEMO: Camera-attached spotlight toggled with 'F'
//
...
void generatePlane(std::vector<Vertex>& verts) {
    float h = 0.5f;
    // Faces -Z to match standard OpenGL light forward vector
    Vec3 n(0, 0, -1);
...
public:
    ...
    void draw(const Frustum& frustum, int& cullCount, int& drawCount) {
        ...
        if (renderable) {
            ...
            // Light Mesh Emissive Logic
            glUniform1i(glGetUniformLocation(g_shaderProgram, "uIsLightMesh"), isLightMesh ? 1 : 0);
            glUniform3f(glGetUniformLocation(g_shaderProgram, "uLightColor"), lightColor.x, lightColor.y, lightColor.z);
...
public:
    ...
    void draw(const Frustum& frustum, int& cullCount, int& drawCount) {
        ...
        glUniform3fv(locCols, visible.size(), colors.data());
...
public:
    ...
    void setupVisuals(Renderable* cube, Renderable* plane) {
        ...
        if (data.type == LIGHT_POINT || data.type == LIGHT_SUN) {
            ...
            visualNode->setRenderable(cube);
            visualNode->scale = Vec3(0.4f, 0.4f, 0.6f); // Slightly elongated
    ...
    void updateWorldTransform(const Mat4& parentWorld) override {
        ...
        // Derive light properties from the Scene Graph world matrix!
...
const char* fragmentShaderSource = R"(
    ...
    // FAKE AREA LIGHT TRICK 1: Epic Games / Unreal Sphere Projection
    vec3 CalcSphereAreaLight(AreaLight light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 baseColor, vec3 specColor, float shininess) {
        ...
        // SPECULAR TRICK: Project reflection vector onto the sphere volume
    ...
    // FAKE AREA LIGHT TRICK 2: Frostbite / Representative Point
    ...
    void main() {
        ...
        vec3 specColor = uMaterial.specular;
        float shininess = max(uMaterial.shininess, 1.0);

        vec3 result = vec3(0.0);
        result += baseColor * 0.05; // Global Ambient

        // Directional (Sun)
        result += CalcDirLight(uDirLight, norm, viewDir, baseColor, specColor, shininess);

        // Point Lights
        for(int i = 0; i < uNumPointLights; i++)
            result += CalcPointLight(uPointLights[i], norm, vFragPos, viewDir, baseColor, specColor, shininess);

        // Spot Lights
        for(int i = 0; i < uNumSpotLights; i++)
            result += CalcSpotLight(uSpotLights[i], norm, vFragPos, viewDir, baseColor, specColor, shininess);

        // Area Lights (The Fake Tricks)
        for(int i = 0; i < uNumAreaLights; i++) {
            if (uAreaLights[i].type == 0) 
                result += CalcSphereAreaLight(uAreaLights[i], norm, vFragPos, viewDir, baseColor, specColor, shininess);
            else if (uAreaLights[i].type == 1) 
                result += CalcRectAreaLight(uAreaLights[i], norm, vFragPos, viewDir, baseColor, specColor, shininess);
        }
...
void drawScene(SceneNode* root) {
    ...
    // Update world transforms and gather active lights
    Mat4 identity;
    ...
    for (auto* ln : g_lightNodes) {
        // --- FLASHLIGHT MOD: Skip hidden lights ---
        ...
        if (ln->data.type == LIGHT_SUN) {
            ...
            // FAKE TRICK 3: CPU-Side Area Light Approximation 
            // Breaks the grid light into 4 smaller point lights dynamically!
    ...
    glUniform1i(glGetUniformLocation(g_shaderProgram, "uNumPointLights"), pCount);
    for(int i=0; i<pCount; ++i) {
        std::string base = "uPointLights[" + std::to_string(i) + "].";
        glUniform3f(glGetUniformLocation(g_shaderProgram, (base+"position").c_str()), pts[i].position.x, pts[i].position.y, pts[i].position.z);
        glUniform3f(glGetUniformLocation(g_shaderProgram, (base+"color").c_str()), pts[i].color.x, pts[i].color.y, pts[i].color.z);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"intensity").c_str()), pts[i].intensity);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"constant").c_str()), pts[i].constant);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"linear").c_str()), pts[i].linear);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"quadratic").c_str()), pts[i].quadratic);
    }

    // Upload Spot Lights
    int sCount = std::min((int)spots.size(), 4);
    glUniform1i(glGetUniformLocation(g_shaderProgram, "uNumSpotLights"), sCount);
    for(int i=0; i<sCount; ++i) {
        std::string base = "uSpotLights[" + std::to_string(i) + "].";
        glUniform3f(glGetUniformLocation(g_shaderProgram, (base+"position").c_str()), spots[i].position.x, spots[i].position.y, spots[i].position.z);
        glUniform3f(glGetUniformLocation(g_shaderProgram, (base+"direction").c_str()), spots[i].direction.x, spots[i].direction.y, spots[i].direction.z);
        glUniform3f(glGetUniformLocation(g_shaderProgram, (base+"color").c_str()), spots[i].color.x, spots[i].color.y, spots[i].color.z);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"intensity").c_str()), spots[i].intensity);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"cutOff").c_str()), spots[i].cutOff);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"outerCutOff").c_str()), spots[i].outerCutOff);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"constant").c_str()), spots[i].constant);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"linear").c_str()), spots[i].linear);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"quadratic").c_str()), spots[i].quadratic);
    }

    // Upload Area Lights
    int aCount = std::min((int)areas.size(), 4);
    glUniform1i(glGetUniformLocation(g_shaderProgram, "uNumAreaLights"), aCount);
    for(int i=0; i<aCount; ++i) {
        std::string base = "uAreaLights[" + std::to_string(i) + "].";
        glUniform3f(glGetUniformLocation(g_shaderProgram, (base+"position").c_str()), areas[i].position.x, areas[i].position.y, areas[i].position.z);
        glUniform3f(glGetUniformLocation(g_shaderProgram, (base+"direction").c_str()), areas[i].direction.x, areas[i].direction.y, areas[i].direction.z);
        glUniform3f(glGetUniformLocation(g_shaderProgram, (base+"right").c_str()), areas[i].right.x, areas[i].right.y, areas[i].right.z);
        glUniform3f(glGetUniformLocation(g_shaderProgram, (base+"up").c_str()), areas[i].up.x, areas[i].up.y, areas[i].up.z);
        glUniform3f(glGetUniformLocation(g_shaderProgram, (base+"color").c_str()), areas[i].color.x, areas[i].color.y, areas[i].color.z);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"intensity").c_str()), areas[i].intensity);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"width").c_str()), areas[i].width);
        glUniform1f(glGetUniformLocation(g_shaderProgram, (base+"height").c_str()), areas[i].height);
        glUniform1i(glGetUniformLocation(g_shaderProgram, (base+"type").c_str()), areas[i].type == LIGHT_AREA_SPHERE ? 0 : 1);
    }
    ...
    std::string title = "Advanced Scenegraph | Drawn: " + std::to_string(drawCount) + " | Culled: " + std::to_string(cullCount) + " | Press 'F' for Flashlight";
...
void handleInput(GLFWwindow* window) {
    ...
    // --- FLASHLIGHT MOD: Toggle Flashlight ---
...
int main() {
    ...
    g_window = glfwCreateWindow(800, 600, "Advanced Scenegraph & Lighting", nullptr, nullptr);
    if (!g_window) {
    ...
    // Setup Renderables for regular objects
    ...
    // Setup Renderables specifically for Light Visualizations
    ...
    // 1. Sun (Directional Light) - DIMMED FOR FLASHLIGHT DEMO
    LightNode* sun = new LightNode(LIGHT_SUN);
    sun->rotation = Vec3(-45, 30, 0);
    sun->data.color = Vec3(0.8f, 0.9f, 1.0f);
    sun->data.intensity = 0.05f; // Reduced to "moonlight" to make flashlight pop
    ...
    // 4. Area Light: Sphere Light (Blue) - Uses Epic Sphere Projection
    ...
    // 5. Area Light: Rect Light (Yellow) - Uses Frostbite Representative Point
    ...
    // 6. Area Light: Grid Light (Magenta) - CPU Trick: Breaks into 4 point lights
    ...
    // --- FLASHLIGHT MOD: 7. Flashlight ---
    g_flashlight = new LightNode(LIGHT_SPOT);
    g_flashlight->data.color = Vec3(1.0f, 1.0f, 0.9f); // Warm white beam
    g_flashlight->data.intensity = 150.0f; // Strong light
    g_flashlight->data.cutOff = cos(12.5f * 3.14159265f / 180.0f); // Tight beam
    ...
    if(g_flashlight->visualNode) {
        // Offset it visually so it looks like you are holding it 1 unit in front of the camera
    ...
    // 8. Animated OBJ Model (Tinted Green)
    ...
    // 9. Instanced Terrain Grid (120 Boxes)
    ...
    while (!glfwWindowShouldClose(g_window)) {
        ...
        // --- FLASHLIGHT MOD: Update position/rotation to match camera ---
        if (g_flashlight) {
