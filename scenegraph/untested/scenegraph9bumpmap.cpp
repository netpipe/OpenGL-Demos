...
// ============================================================================
// Compile (Windows): g++ scenegraph7.cpp -o demo -lglfw3 -lglew32 -lopengl32
// Compile (Linux):   g++ scenegraph7.cpp -o demo -lglfw -lGLEW -lGL
//
    ...
    void loadMTL(const std::string& filename) {
        ...
        while (std::getline(file, line)) {
            ...
            // --- ADDED NORMAL MAP PARSING ---
                ...
                std::string bumpPath;
                ss >> bumpPath;
                current->normalMap = basePath + bumpPath;
            }
        }
    }
...
void generateBox(std::vector<BumpVertex>& verts) { // UPDATED
    ...
    auto addQuad = [&](Vec3 p0, Vec3 p1, Vec3 p2, Vec3 p3, Vec3 n) {
        Vec3 t(1,0,0), b(0,1,0); // Simple default tangent space for boxes
...
void loadModelIntoRenderable(Renderable* r, const std::string& path) {
    ...
    if (model.load(path) && !model.meshes.empty()) {
        ...
        r->addMesh(boxMesh, tex, 0, true); // UPDATED
    }
}
...
public:
    ...
    void setupVisuals(Renderable* cube, Renderable* plane) {
        ...
        if (data.type == LIGHT_SUN || data.type == LIGHT_AREA_RECT || data.type == LIGHT_AREA_GRID) {
...
int main() {
    ...
    Renderable* boxRenderable = new Renderable();
    {
        ...
        boxRenderable->addMesh(boxMesh, boxTex, 0, true);
    }
