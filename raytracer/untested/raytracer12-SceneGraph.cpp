// ============================================================
// GPU PERCEPTUAL RAY TRACER + BVH + SCENE GRAPH - OpenGL 4.1 / macOS
// ============================================================
...
// - GLFW input/window
// - GLEW loading
// - Hierarchical scene graph (SceneNode / GroupNode / SphereNode)
// - BVH construction/upload (rebuildable at runtime for CPU animation)
// - PNG screenshots

// The scene graph is authored as a tree of SceneNode objects with local
// transforms. flattenSceneGraph() walks the tree, resolves world-space
// transforms, and fills the flat Sphere array that the BVH builder and
// the GPU scene textures consume. The GPU pipeline is unchanged.

// The BVH is built on the CPU. Spheres and BVH nodes are uploaded
...
// macOS build example:
// clang++ raytracer11-settings.cpp -std=c++17 -O2 \
//     -lglfw -lGLEW -lpng -framework OpenGL -o raytracer11
...
// T Toggle shader-animated ball
// G Toggle CPU scene-graph animation (rebuilds BVH each frame)
// ESC Quit
// ============================================================
...
static constexpr float FOVEA_RADIUS = 0.40f;
static constexpr float MID_RADIUS = 0.60f;
static constexpr int FOVEA_SPP = 1;
static constexpr int BURST_FOVEA_SPP = 3;
static constexpr int BURST_MID_SPP = 1;
...
static constexpr int GODRAY_STEPS = 3;
...
static constexpr float ANIMATION_REFRESH_MIN_CONTRIBUTION = 0.035f;
...
static constexpr float ANIMATION_FULL_REFRESH_INTERVAL = 10.25f;
...
// Scene graph
// ============================================================
// A minimal transform hierarchy. Nodes carry a local translation and
// scale; world transforms are resolved by walking up the parent chain.
// Geometry nodes override collectSpheres() to emit themselves (in world
// space) into the flat arrays consumed by the BVH builder and the GPU.
class SceneNode
{
public:
    std::string name;
    ...
    Vec3 getWorldPosition() const
    {
        ...
        while (p)
        {
            pos.x *= p->localScale.x;
            pos.y *= p->localScale.y;
            pos.z *= p->localScale.z;
            pos = pos + p->localPosition;
            p = p->parent;
        }
        return pos;
    }
    ...
    virtual void collectSpheres(std::vector<Sphere>& outSpheres,
                                std::vector<SceneNode*>& outNodes)
    {
        for (auto c : children) c->collectSpheres(outSpheres, outNodes);
    }
};

// An empty transform group: useful for assembling sub-hierarchies
// (vehicles, solar systems, characters) that move/scale as one unit.
class GroupNode : public SceneNode
{
public:
    GroupNode(const std::string& nodeName = "Group") { name = nodeName; }
};
...
public:
    ...
    SphereNode(float r, const Material& m, const std::string& nodeName = "Sphere")
        : radius(r), material(m) { name = nodeName; }

    void collectSpheres(std::vector<Sphere>& outSpheres,
                        std::vector<SceneNode*>& outNodes) override
    {
        Vec3 worldPos = getWorldPosition();
        Vec3 worldScale = getWorldScale();
        // The tracer intersects spheres, not ellipsoids: use the largest
        // scale axis so non-uniform parent scales stay conservative.
        float maxScale = std::max({worldScale.x, worldScale.y, worldScale.z});

        outSpheres.push_back({worldPos, radius * maxScale, material});
        outNodes.push_back(this);

        SceneNode::collectSpheres(outSpheres, outNodes);
    }
};

static SceneNode* sceneRoot = nullptr;
static std::vector<SceneNode*> sphereNodes;    // parallel to `spheres`
static SphereNode* shaderAnimatedNode = nullptr;

// Nodes used by the optional CPU-driven graph animation (key G).
static GroupNode* solarSystemNode = nullptr;
static SphereNode* orbitPlanetNode = nullptr;
static SphereNode* orbitMoonNode = nullptr;
static bool cpuGraphAnimationEnabled = false;

// Walks the graph and rebuilds the flat sphere array (+ the parallel
// node list) used by buildBVH() and the GPU scene textures.
static void flattenSceneGraph()
{
    spheres.clear();
    sphereNodes.clear();
    if (sceneRoot) sceneRoot->collectSpheres(spheres, sphereNodes);

    // Map the shader-animated node back to its flat-array index so the
    // BVH can keep conservative bounds over its motion path.
    animatedSphereOriginalId = -1;
    for (size_t i = 0; i < sphereNodes.size(); ++i)
    {
        if (sphereNodes[i] == shaderAnimatedNode)
        {
            animatedSphereOriginalId = (int)i;
            break;
        }
    }
}
...
static void computeNodeBounds(int nodeIndex, int first, int count)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        // The shader-animated sphere is moved in the fragment shader.
        // Keep its BVH bounds conservative over the complete motion path
        // so the GPU never culls it while it moves.
...
static void buildBVH()
{
    ...
    animatedSphereTextureIndex = -1;
    if (animatedSphereOriginalId >= 0)
    {
...
bool hitSphere(vec3 ro,vec3 rd,int sphereIndex,inout Hit h)
{
    ...
    vec3 c=cr.xyz;
    float r=cr.w;
    // Animate the flagged sphere in the compact BVH texture. Its CPU-side
...
// Scene textures
// ============================================================
// Uploads the current flat sphere/BVH arrays to the GPU textures.
// Safe to call at any time (e.g. after a CPU scene-graph update).
static void updateSceneTextures()
{
...
// ============================================================
// Scene setup (authored through the scene graph)
// ============================================================
...
static void setupScene()
{
    ...
    sceneRoot = new GroupNode("Root");
    solarSystemNode = nullptr;
    orbitPlanetNode = nullptr;
    orbitMoonNode = nullptr;
    shaderAnimatedNode = nullptr;

    // --- Original four hero objects, now as graph leaves ---
    auto* redSphere = new SphereNode(1.0f,
        makeMaterial(Vec3(0.85f,0.08f,0.07f), 0.72f, 0.0f, 1.0f), "Red Sphere");
    redSphere->localPosition = Vec3(-1.45f, 1.0f, 0.0f);

    auto* blueSphere = new SphereNode(1.0f,
        makeMaterial(Vec3(0.08f,0.20f,0.85f), 0.68f, 0.0f, 1.0f), "Blue Sphere");
    blueSphere->localPosition = Vec3(1.15f, 1.0f, -0.4f);

    auto* greenSphere = new SphereNode(0.72f,
        makeMaterial(Vec3(0.10f,0.78f,0.18f), 0.64f, 0.0f, 1.0f), "Green Sphere");
    greenSphere->localPosition = Vec3(0.0f, 0.72f, 1.55f);

    // Crystal-clear glass orb.
    auto* glassSphere = new SphereNode(1.15f,
        makeMaterial(Vec3(0.96f,0.98f,1.0f), 0.0f, 1.0f, 1.5f), "Glass Orb");
    glassSphere->localPosition = Vec3(0.0f, 1.15f, -1.65f);

    sceneRoot->addChild(redSphere);
    sceneRoot->addChild(blueSphere);
    sceneRoot->addChild(greenSphere);
    sceneRoot->addChild(glassSphere);

    // --- Scene-graph hierarchy demo: a small solar system ---
    // Moving/scaling solarSystemNode moves the sun, planet AND moon
    // together; moving the planet carries the moon with it.
    auto* solarSystem = new GroupNode("Solar System");
    solarSystem->localPosition = Vec3(-4.0f, 2.5f, -2.0f);
    sceneRoot->addChild(solarSystem);

    auto* sun = new SphereNode(0.5f,
        makeMaterial(Vec3(1.0f,0.8f,0.2f), 0.0f, 0.0f, 1.0f, 3.0f), "Sun");
    solarSystem->addChild(sun);

    auto* planet = new SphereNode(0.2f,
        makeMaterial(Vec3(0.2f,0.6f,0.8f), 0.5f, 0.0f, 1.0f), "Planet");
    planet->localPosition = Vec3(1.2f, 0.0f, 0.0f);   // relative to solarSystem
    solarSystem->addChild(planet);

    auto* moon = new SphereNode(0.05f,
        makeMaterial(Vec3(0.8f,0.8f,0.8f), 0.9f, 0.0f, 1.0f), "Moon");
    moon->localPosition = Vec3(0.4f, 0.0f, 0.0f);     // relative to planet
    planet->addChild(moon);

    solarSystemNode = solarSystem;
    orbitPlanetNode = planet;
    orbitMoonNode = moon;

    // The sphere the fragment shader animates (originally spheres[0]).
    // Its BVH bounds are automatically expanded over the motion path.
    shaderAnimatedNode = redSphere;

    // The infinite floor is hard-coded in the shader (y = 0); keep its
    // description here for completeness / future shader upgrades.
    ...
    // Resolve the hierarchy into the flat arrays for BVH + GPU upload.
    flattenSceneGraph();
}
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(
        ...
        "GPU Perceptual Ray Tracer + BVH + Scene Graph (OpenGL 4.1)",
        nullptr, nullptr);
    if (!window)
    {
    ...
    setupScene();          // builds the scene graph and flattens it
    buildBVH();            // builds BVH over the flattened spheres
    if (!initShaders())
    {
        cleanup();
        glfwDestroyWindow(window);
    ...
    initSceneTextures();   // allocates + uploads scene/BVH textures
    initAccumulation();
    glGenVertexArrays(1, &displayVAO);
 //   clearAccumulation();
         if (cpuGraphAnimationEnabled)
        {
            float t = (float)glfwGetTime();
            if (solarSystemNode)
                solarSystemNode->localPosition =
                    Vec3(-4.0f, 2.5f + std::sin(t*0.6f)*0.35f, -2.0f);
            if (orbitPlanetNode)
                orbitPlanetNode->localPosition =
                    Vec3(std::cos(t*0.9f)*1.2f, 0.0f, std::sin(t*0.9f)*1.2f);
            if (orbitMoonNode)
                orbitMoonNode->localPosition =
                    Vec3(std::cos(t*2.3f)*0.4f, 0.0f, std::sin(t*2.3f)*0.4f);
            flattenSceneGraph();
            buildBVH();
            updateSceneTextures();
            // IMPORTANT: do NOT call clearAccumulation() here. That would reset
            // frameIndex to 0 every frame, permanently re-enabling the expensive
            // burst-SPP schedule and pushing older GPUs past Apple's watchdog
            // timeout (GPU hang). Discard history only, keep the SPP schedule.
            accumulationReset = true;
        }    std::printf(
        "\n==================================================\n"
        " GPU PERCEPTUAL RAY TRACER + BVH + SCENE GRAPH\n"
        ...
        "P Save screenshot.png\n"
        "T Toggle shader-animated ball\n"
        "G Toggle CPU scene-graph animation\n"
        "ESC Quit\n"
        "\n"
        "Hierarchical scene graph enabled (GroupNode/SphereNode).\n"
        ...
        "CPU scene-graph animation rebuilds the BVH per frame (G).\n"
        ...
        WINDOW_WIDTH, WINDOW_HEIGHT,
        TARGET_FPS, FOVEA_RADIUS, FOVEA_SPP, BURST_FOVEA_SPP, BURST_MID_SPP, MAX_BOUNCES,
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (tDown && !previousT)
        {
            ...
            std::printf("Shader animation %s.\n", animationEnabled ? "enabled" : "paused");
        }
        previousT = tDown;
        bool gDown = glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS;
        if (gDown && !previousG)
        {
            cpuGraphAnimationEnabled = !cpuGraphAnimationEnabled;
            clearAccumulation();
            std::printf("CPU scene-graph animation %s.\n",
                        cpuGraphAnimationEnabled ? "enabled" : "paused");
        }
        previousG = gDown;
        // Optional CPU-driven scene-graph animation demo: orbit the planet
        // around the sun and the moon around the planet by editing local
        // transforms, then re-flatten the graph, rebuild the BVH and
        // re-upload the scene textures. NOTE: moving geometry invalidates
        // temporal history, so this path restarts accumulation every frame
        // and is noisier than the shader-animated sphere (key T).
        if (cpuGraphAnimationEnabled)
        {
            float t = (float)glfwGetTime();
            if (solarSystemNode)
