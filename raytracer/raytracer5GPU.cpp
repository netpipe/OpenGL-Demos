...
vec3 tracePath(vec3 ro,vec3 rd,inout uint state)
{
    ...
    for(int bounce=0;bounce<16;bounce++)
    {
        ...
        if(h.transmission>0.5)
        {
            ...
            // Russian Roulette: Choose between Reflection or Refraction
            if (rand(state) < F) {
                // --- REFLECTION PATH ---
                ...
                if(refractDir(rd, h.normal, h.ior, refrDir)) {
                    Hit inside;
                    // Trace ray through the glass to find the back face
                    if(intersectScene(h.position - refrDir * EPS, refrDir, inside)) {
                        vec3 exitDir;
                        // Attempt to exit the glass back into the air
                        if(refractDir(refrDir, inside.normal, inside.ior, exitDir)) {
                            ro = inside.position + exitDir * EPS;
                            rd = exitDir;
                            throughput *= h.albedo;
                        } else {
                            // Total Internal Reflection (TIR) at the back face
                            rd = normalize(reflectDir(refrDir, inside.normal));
                            ro = inside.position + inside.normal * EPS;
                            throughput *= h.albedo;
                        }
                    } else {
                        break; // Ray missed the back of the glass (rare)
                    }
                } else {
                    break; // Refraction failed
                }
            }
            
            // CRITICAL FIX: Use 'continue' instead of 'break' so the ray 
            // continues into the scene behind the glass!
            continue; 
        }
